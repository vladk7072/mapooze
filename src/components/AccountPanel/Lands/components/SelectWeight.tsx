...
import { dataWeight } from "../lands.data";
import { useAppDispatch, useAppSelector } from "../../../../hooks/redux-hooks";
import { myLandsSlice } from "../../../../store/slices/myLands.slice";

export const SelectWeight: FC = () => {
  const { selectFont, selectWeight, isActiveWeightList } = useAppSelector(
    (state) => state.myLandsReducer
  );
  const { setSelectWeight, setActiveWeightList } = myLandsSlice.actions;
  const dispatch = useAppDispatch();

  const selectLandHandler = (idx: number) => {
    dispatch(setSelectWeight(idx));
    ...
    <div className="lands__input lands__input-list lands__input--second">
      <div className="saleplace__general-trial saleplace__general-trial-select exchange__select">
        <div
          ...
          onClick={() => dispatch(setActiveWeightList(true))}
        >
          <span
            className={
              isActiveWeightList || selectFont
          ...
          {!!selectFont && (
            <div className="addwallet__select-title">
              {dataWeight[selectFont].weights[selectWeight ? selectWeight : 1]}
            </div>
          )}
        </div>
        {isActiveWeightList && selectFont !== null && (
          <ul className="saleplace__general-select-list addwallet__list lands__list">
            {dataWeight[selectFont].weights.map((el, idx) => (
              <li
                className={
                  selectWeight === idx
